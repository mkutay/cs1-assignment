...
.equ NUMBER_ONE, 0x13 ; register 19 - the constant 0x01
.equ MORSE_COUNTER, 0x17 ; register 23 - the counter for displaying the morse code, up until 50
.equ DIV_BY_FIVE_COUNTER, 0x16 ; register 22 - the counter used for finding the indexes that are divisible by 5
.equ FLIP_BIT, 0x11 ; register 17 - to cycle through the morse code data to make the leds ON or OFF
...
main: ; the main function
  ...
  ; NOTE: we only want single bytes from the memory, but the Z index can hold up to two bytes, so we only set the LOWER byte of the Z index
...
not_even:
  ...
  ldi FLIP_BIT, 0x01
  ldi ZL, morse_five ; set Z index to the first address of morse code of five
...
delay:
  ...
  ldi r25, 202
inner_delay:
  dec r25 ; 1 instruction, if r25 equals to 0, the register underflows to 255
...
; we can hold our data values in memory as single bytes, and by adding a zero at the end,
...
; finally, the instructions that were used was informed by these pages: http://www.rjhcoding.com/avr-asm-sram.php and http://www.rjhcoding.com/avr-asm-pm.php
...
; the second byte will be used as a delay to display the led OFF, and so on... until we reach the end
